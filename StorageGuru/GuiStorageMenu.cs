        ...
        public void Update()
        {
            if (NeedsRefresh)
            {
                ...
                if (ActiveStorageModule.GetResourceStorageObject() is ImprovedResourceStorage improvedResourceStorage)
                {
                    ...
                    EnableAll = entry.Count != StorageGuru.MasterResourceDefinitions.Count && (entry.Count == 0 || EnableAll);

                    clear();

                    // Add toggle for each reasource + style based on whether enabled
                    foreach (var resourceType in StorageGuru.MasterResourceDefinitions)
                    {
